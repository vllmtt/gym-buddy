...
import kotlinx.android.synthetic.main.activity_cardio_exercise_editor.*

class CardioExerciseEditor : AppCompatActivity() {
    ...
    private var exercise: CardioType? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_cardio_exercise_editor)
    ...
    private fun fetchExercise() {
        ...
        if (exerciseId != null && exerciseId > -1L) {
            exercise = GymBuddyRoomDataBase.cardioTypeDao.get(exerciseId)
            id = exercise?.id
            name = exercise?.name
        }
    }
    ...
    private fun save(): Boolean {
        ...
        if (exercise == null) {
            exercise = CardioType(0, name)
            AsyncTask.execute {
                GymBuddyRoomDataBase.cardioTypeDao.insertAll(exercise)
            }
        } else {
            exercise.name = name
            AsyncTask.execute {
                GymBuddyRoomDataBase.cardioTypeDao.updateAll(exercise)
            }
        }
